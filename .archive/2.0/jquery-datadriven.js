...
;(function($) {
  ...
  $.datepicker.setDefaults({
    buttonImage: '/boilerplate/master/css/img/datepicker.gif',
...
(function($) {
  ...
  $.ui.accordion.prototype._create = function(){
    ...
    // override options with data-*
      options = {
        ...
        icons: !_element.data('ui-icons') ? _element.data('ui-icons') : {
  ...
  $.ui.autocomplete.prototype._create = function(){
    ...
    // get data source
      source = _element.data('ui-autocomplete-source'),
    ...
    // override options with data-*
      options = {
        ...
        source: source  // pass array or string
      };

    // source reference not an array
    if (typeof source === "string") {
      _element.on('click focus', function() {
        $(_element).autocomplete( "option", "source", window[source] || $.ddui.helpers.stringToMethod(source) || source );
      });
    }
  ...
  $.ui.button.prototype._create = function(){
    ...
    if (_element.data('ui-loading-text')) {
  ...
  $.ui.button.prototype.loading = function(){
    ...
    // get loading text
      loadingText = _element.data('ui-loading-text') || 'Loading...';
        ...
        getOptions: function($target, inst) {
          ...
          var maxDate = $.ddui.helpers.splitString($target.data('ui-datepicker-maxdate'), ',', true),
              ...
              beforeShow = function(el) {
                ...
                // save default max and min dates as data attribute, to reference in subsequent uses
                $(el).data('default-maxdate', $(el).data('default-maxdate') || _datepicker._determineDate(currMaxdate));
                $(el).data('default-mindate', $(el).data('default-mindate') || currMindate ? (typeof _datepicker === 'string' ? _datepicker._determineDate(currMindate) : currMindate) : 'no default mindate');
                ...
                mindate = mindate < new Date($(el).data('default-mindate')) || mindate === null ? $(el).data('default-mindate') : mindate;
  ...
  $.ddui = {
    ...
    _init: function(sel) {
      if (sel) {
        ...
        $(sel)
          .find('[data-role]')
            ...
            .filter('[data-role="autocomplete"]').autocomplete()
              .end()
            .filter('[data-role="button"]').button().filter('[data-ui-loading-text]').on('click', function(){ var $this = $(this); $this.button('loading'); }).end()
              .end()
            .filter('[data-role="buttonset"]').buttonset().filter('[data-ui-loading-text]').on('click', function(ev){ var $this = $(this); $this.buttonset('loading', ev); }).end()
              .end()
            .filter('[data-role="datepicker"]').datepicker()
              .end()
            .filter('[data-role="dialog"]').dialog()
              .end()
            .filter('[data-role="menu"], [data-role="menu-button"]').menu().on('mouseleave', function() { $(this).menu( 'collapseAll', null, true ); })
              .end()
        ...
        // global init
        $('[data-role]')
          .filter('[data-role="accordion"]').accordion()
            .end()
          .filter('[data-role="autocomplete"]').autocomplete()
            .end()
          .filter('[data-role="button"]').button().filter('[data-ui-loading-text]').on('click', function(){ var $this = $(this); $this.button('loading'); }).end()

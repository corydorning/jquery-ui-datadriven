...
;(function($) {
  ...
  // accordion
  $.ui.accordion.prototype.options.header = '> section > h3';
...
(function($) {
  ...
  $.ui.accordion.prototype._create = function(){
    ...
    // override options with data-*
      options = {
        active: _element.data('ui-accordion-active') !== undefined ? _element.data('ui-accordion-active') : _this.options.active,
  ...
  $.datepicker.constructor.prototype._connectDatepicker = function(target, inst) {
    var _this = this,
      $target = $(target),
      setdate = $target.data('ui-datepicker-setdate');
    ...
    // set date if needed
    if (setdate){
      $target.datepicker('setDate', setdate);
    }
  ...
  $.ddui = {
    ...
    update: function(modules, scope) {
      var widget = $.ddui.helpers.splitString(modules, ' ', true);
